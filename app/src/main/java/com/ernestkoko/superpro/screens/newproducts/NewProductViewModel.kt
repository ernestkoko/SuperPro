...
//        )
    ...
    //called to save to firebase database
    fun saveToDb() {
        val firebaseProduct = FirebaseProducts(
            _prodName,
            3.toString(),
            _prodExpiryDate.toString(),
            _prodManufacturer,
            ""
        )
        repository.insertIntoFirebase(firebaseProduct)
       // database.push().setValue(firebaseProduct)
        Log.i(TAG, "Inserted into firebase")
